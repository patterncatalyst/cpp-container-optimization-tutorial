...
# audience.
#
#   ./scripts/pre-pull.sh           # pull everything
#   ./scripts/pre-pull.sh --prune   # pull, then `podman image prune` (recover space)
#
...
if (( failed > 0 )); then
    log_err "$failed image(s) failed to pull:"
    for image in "${failed_images[@]}"; do
        log_err "  - $image"
    done
    log_warn "If a docker.io image failed, see §1 Prerequisites for the"
    log_warn "podman save/load air-gap workaround."
    exit 1
fi
