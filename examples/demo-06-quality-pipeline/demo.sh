...
if [[ $DO_CLEAN -eq 1 ]]; then
  ...
  podman rmi -f \
    cpp-tut/demo-06:analyzer \
    cpp-tut/demo-06:tests \
    cpp-tut/demo-06:abi \
    cpp-tut/demo-06:svc \
    cpp-tut/demo-06:gdbserver 2>/dev/null || true
  rm -rf reports
  log_ok "Cleaned."
  exit 0
fi
...
run_phase() {
  ...
  podman build --target "$phase" -t "cpp-tut/demo-06:$phase" .
  # Pull the reports out of the image so the host sees them.
  local cid
  cid=$(podman create "cpp-tut/demo-06:$phase")
