...
The bottom two rows on threading state are worth highlighting: `thread_local` looks request-scoped to a developer reading the code, and it is one of the most common silent-bug sources. The `CorrelationGuard` pattern from [Doc 05](../05-threading/) puts a `thread_local` write inside an RAII type whose scope is the handler, which is the legitimate way to use TLS in a request handler.
...
11. **OTel + Grafana stack provides external observability that makes statelessness work.** When OS containers are interchangeable, logs/metrics/traces aggregate externally. The OTel `TracerProvider` is process-scoped ([Doc 04](../04-process-scoped-state/)); span context is request-scoped ([Doc 02](../02-raii/)); the OTel `Scope` type is the TLS guard pattern ([Doc 05](../05-threading/)) the framework itself uses.

## Glossary

**Request scope.** The lifetime of a single inbound RPC, bounded by the handler's stack frame (or the reactor's lifetime for streaming RPCs). State here lives in `RequestContext` members or PMR arenas; destroyed on handler return. [Doc 02](../02-raii/), [Doc 03](../03-pmr/).

**Process scope.** The lifetime of the OS container process, bounded by `main()`'s scope. State here is constructed once at startup, lives across many requests, destroyed at process exit. [Doc 04](../04-process-scoped-state/).

**Deploy-time scope.** The lifetime of a particular deployment, often longer than any single container instance. Configuration values that vary between dev and production but not between two replicas of the same service live here, in environment variables or config maps. [Doc 06](../06-twelve-factor/).

**External state.** State that lives outside any process — Redis, PostgreSQL, Kafka, S3. Lifetime decoupled from any one container; authoritative for cross-replica consistency. [Doc 07](../07-state-externalization/).
...
**Stateless deployment posture.** A deployment configuration under which the orchestrator can kill any replica without correctness loss, because any authoritative state is externally stored and process-scoped state is rebuildable from configuration. [Doc 01](../01-deployment-posture/).

## Reading paths by scenario

**Building a new C++ gRPC service from scratch.** Read [Doc 01](../01-deployment-posture/) (vocabulary), [Doc 02](../02-raii/) (RAII), [Doc 04](../04-process-scoped-state/) (process state + table), [Doc 09](../09-health-checks/) (health checks), [Doc 10](../10-grpc-microservices/) (integration). Use [Doc 11](../11-build-tooling/) as the build-tooling reference while setting up the project. [Doc 03](../03-pmr/), 05, 06, 07, 08 can be read as needed when specific concerns surface.

**Migrating a C++ monolith to containers.** Read [Doc 01](../01-deployment-posture/) (especially the monolith-intuitions section), [Doc 06](../06-twelve-factor/) (12-factor adaptation, where most of the migration tension lives), [Doc 07](../07-state-externalization/) (state externalization, the biggest mechanical refactor), [Doc 08](../08-ephemeral-filesystem/) (ephemeral filesystem — the silent bug source in migrations). [Doc 04](../04-process-scoped-state/) establishes the State Architecture Table that drives the audit.

**Troubleshooting tail-latency spikes.** Read [Doc 05](../05-threading/) first (CFS throttling, oversubscription, thread pool sizing). Then [Doc 03](../03-pmr/) (PMR destruction asymmetry, allocator contention). [Doc 04](../04-process-scoped-state/)'s CPU-limits section gives the budget framing.

**Troubleshooting OOM kills or memory bloat.** Read [Doc 04](../04-process-scoped-state/) (memory limits as the hard ceiling, the bounded-vs-unbounded audit). Then [Doc 05](../05-threading/)'s allocator-arena section (jemalloc/tcmalloc/glibc arena counts under cgroup limits). Then [Doc 03](../03-pmr/) (PMR upstream resource sizing, inline buffer tuning).

**Troubleshooting deploy failures or rolling-restart issues.** Read [Doc 09](../09-health-checks/) (probes, especially the liveness-checks-downstream anti-pattern). [Doc 06](../06-twelve-factor/)'s disposability section covers the C++ startup tax that affects probe timing.

**Troubleshooting "logs disappear after restart" or filesystem-related issues.** [Doc 08](../08-ephemeral-filesystem/) directly.

**Adopting C++23 in an existing project.** [Doc 11](../11-build-tooling/)'s feature-support table maps requirements to compiler versions. [Doc 02](../02-raii/) and [Doc 03](../03-pmr/) cover the specific C++23 features used across the doc set (`std::expected`, `<stacktrace>`, `std::pmr::stacktrace`, `std::flat_map`).

**Just need the gRPC patterns.** [Doc 10](../10-grpc-microservices/) is the integration; [Doc 05](../05-threading/) has the threading model and the gRPC server sizing trap; [Doc 07](../07-state-externalization/) has the deadline-propagation pattern; [Doc 09](../09-health-checks/) has the health protocol.
...
**Other backing services.** MongoDB, Cassandra, ScyllaDB, Elasticsearch, ClickHouse. The connection-pool and deadline-propagation patterns from [Doc 07](../07-state-externalization/) apply; the specific C++ client libraries vary in quality and Conan availability.

**Service mesh integration.** Istio, Linkerd, Cilium. The mesh handles much of what [Doc 07](../07-state-externalization/)'s retry-with-backoff and [Doc 09](../09-health-checks/)'s health-check coordination cover at the transport layer; the C++ service's job becomes simpler under a mesh, but the mesh adds operational complexity that's its own topic.

**Authentication and authorization in depth.** mTLS for gRPC, OAuth2/OIDC token validation, RBAC. [Doc 10](../10-grpc-microservices/)'s `InsecureServerCredentials` placeholder is replaced with `SslServerCredentials` in production; the rest of the patterns are independent of auth.
...
**CI/CD pipelines.** The build artifacts produced by [Doc 11](../11-build-tooling/)'s pipeline plug into Tekton, ArgoCD, GitHub Actions, GitLab CI, Jenkins. The build-side concerns are addressed; the deploy-side automation is platform-specific.

**Production debugging tools.** `perf`, eBPF/bpftrace, `gdb` against core dumps, ASan in production. Useful tools, separate topic.

**Database schema migration.** Liquibase, Flyway, hand-rolled. [Doc 07](../07-state-externalization/)'s admin-process mention in the 12-factor tour is the only coverage; full migration tooling is its own document.
...
**Pekka Enberg, *Latency: Reduce Delay in Software Systems* (Pragmatic, 2024).** Framing of tail latency as a first-class property, with chapters on allocator predictability, threading under throttling, and async I/O. Cited in [Doc 03](../03-pmr/), 04, 05, 07.
...
**The Twelve-Factor App (`12factor.net`, Heroku, 2011/2017).** The canonical 12-factor statement. [Doc 06](../06-twelve-factor/) is essentially the C++ adaptation; [Doc 08](../08-ephemeral-filesystem/) covers Factor XI (Logs) in operational depth; [Doc 09](../09-health-checks/) covers Factor IX (Disposability) on the shutdown side.
